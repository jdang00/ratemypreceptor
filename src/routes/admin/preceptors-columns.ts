...
export const preceptorsColumns: ColumnDef<Preceptor>[] = [
	...
	},
	{
		...
		cell: ({ row }) => {
			const siteSnippet = createRawSnippet<[string]>((getSiteName) => {
				const siteName = getSiteName();
				return {
					render: () => `<div class="text-sm">${siteName}</div>`
				};
			});
			return renderSnippet(siteSnippet, row.getValue('siteName'));
		}
	},
	{
	...
	},
	{
		...
		header: 'Actions',
		cell: ({ row }) => {
			const actionsSnippet = createRawSnippet<[Preceptor]>((getPreceptor) => {
					...
					render: () => `
						<div class="flex items-center gap-2">
							<button 
								...
								data-edit-preceptor='${JSON.stringify(preceptor)}'
								title="Edit"
								type="button"
							>
							...
							<button 
								...
								data-delete-preceptor="${preceptor._id}"
								title="Delete"
								type="button"
							>
